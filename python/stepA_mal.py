    ...
    while True:
        ...
        if isinstance(ast, list):
            if len(ast) == 0:
                ...
                first = ast[0]
                symbols = ["def!", "defmacro!", "let*", "do", "if", "fn*", "quote", "quasiquote", "macroexpand", "try*", "py*", "py!*"]
                if isinstance(first, MalSymbol) and first.name in symbols:
                    if first.name == "def!":
                        ...
                        for e in py_locals:
                            exec(f"{e} = {py_locals[e]}")
                        arg = ast[1].string
                        arg = arg.replace("\\\'", "\"")
                        result = eval(arg)
                        if isinstance(result, int):
                            return MalSymbol(str(result))
                        ...
                        arg = ast[1].string
                        arg = arg.replace("\\\'", "\"")
                        pre_locals = list(locals().keys())
                        exec(arg)
                        post_locals = list(locals().keys())
                        for e in post_locals:
                            if e not in pre_locals:
                                py_locals[e] = locals()[e]
                        return MalNil()
                else:
                    rn = eval_ast(ast, env)
...
EVAL(READ("(def! *host-language* \"python\")"), repl_env)
